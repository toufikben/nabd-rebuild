...
## تحديث hardening — 2026-09-22

أُغلقت في هذه الدورة ثلاث نقاط كانت تمنع اعتبار إدارة قاعدة البيانات مستقرة وظيفيًا. أولًا، أزيلت رسائل `SnackBar` التي كانت تُرسل بعد فجوات غير متزامنة من FilePicker أو عمليات النسخ والاستعادة، واستُبدلت برسالة حالة داخلية في شاشة Settings؛ وبذلك لا تعتمد النتيجة على `BuildContext` أو `ScaffoldMessenger` عابر بعد تغيّر شجرة الواجهة. ثانيًا، أضيف `DatabaseService.dataRevision` كقناة تغيير مركزية، وتُطلق بعد عمليات Hive الأساسية وبعد Restore الناجح، بينما يستمع جذر التطبيق إليها لإعادة بناء الصفحات المعتمدة على البيانات فورًا. ثالثًا، جرى استبدال صوت Splash بصوت قطرة ماء قصير ذي صدى هادئ، مع إبقاء المدة تحت ثلاث ثوانٍ.

هذه التغييرات **لم تُثبت بعد على هاتف فعلي أو عبر Flutter build في هذه البيئة**؛ أدوات `flutter` و`dart` غير مثبتة في الـsandbox الحالي. لذلك تبقى حالة Runtime ونتيجة عدم ظهور `_dependents.isEmpty` بحاجة إلى تحقق ميداني فعلي، ولا تُصنّف هذه الوثيقة أي سيناريو Runtime على أنه PASS دون دليل.

### الخطوة المباشرة التالية

1. بناء وتثبيت APK من commit هذه الدورة على الهاتف الفعلي.
2. تنفيذ Backup Database ثم Restore Database في وضعي Merge وReplace، ومراقبة عدم ظهور شاشة حمراء، ثم التأكد من ظهور الإدخالات المستعادة فورًا في Journal دون إعادة تشغيل التطبيق.
3. اختبار Automatic Database Backup بالمسار الافتراضي، ثم التحقق من ملف `.nabd` وإشعار النجاح أو الفشل.
4. تنفيذ Runtime flow منفصل لـ App Lock وR-Settings & Lock؛ نجاح الإقلاع أو التثبيت وحده لا يغلق هذا المسار.
5. بعد إغلاق التحقق الميداني، الانتقال إلى Key Rotation الآمن، ثم release signing وRelease AAB وstore checklist.

### حالة الأدلة في هذه الدورة

| البند | الحالة الحالية | الدليل أو القيد |
|---|---|---|
| إصلاح شاشة `_dependents.isEmpty` في مسارات Settings | مُطبق في الكود | استبدال `ScaffoldMessenger` برسالة داخلية آمنة؛ يحتاج اختبار جهاز |
| تحديث Journal والصفحات بعد Restore | مُطبق في الكود | `DatabaseService.dataRevision` + إعادة بناء الجذر بعد Restore |
| صوت Splash بصدى قصير | مُطبق | `assets/sounds/splash_water_drop.mp3`؛ يحتاج سماعًا على جهاز |
| Flutter analyze/build في هذا sandbox | غير متاح | `flutter` و`dart` غير موجودين في البيئة الحالية |
| Runtime physical evidence لهذه الدورة | غير متاح بعد | مطلوب تنفيذ المستخدم على الهاتف أو CI Flutter-enabled |

## حالة التسليم بعد هذه الدورة

التغييرات محصورة في `qa/runtime-emulator` ولم يُنفذ merge إلى `main`. لا ينبغي تحديث نسب الجاهزية النهائية أو إعلان Release Candidate قبل نجاح التحقق الميداني أعلاه.
